...
(function($) {
		...
		init: function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					swf: '/Scripts/source/uploadify.swf', // The path to the uploadify SWF file
		...
		onSelect: function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件命名 "' + file.name + '" 已经在队列中.\n是否替换现有的队列项?');
					if (!replaceQueueItem) {
	...
	/*  iss.uploadify = function(method) {
		//_dialog();
		var ele = $(".iss-upload");
		if (methods[method]) {
			return methods[method].apply(ele, Array.prototype.slice.call(arguments, 1));
		} else if (typeof method === 'object' || !method) {
			return methods.init.apply(ele, arguments);
		} else {
			$.error("上传控件加载失败");
		}
	...
	iss.upload = function(ele) {
		// console.log( arguments.callee.caller );
		id = "iss-upload" + new Date().getTime()
		var flotDiv = '<div class="iss-upload_wap"><h4 class="iu_title"></h4><div class="box"><input type="button" class="iss-upload btn90" id="' + id + '" /></div></div>'
		var e = ele ? ele : "";
		iss.dialog.alert({
			content: flotDiv,
			okValue: "关闭",
	...
	$.each(iss._fn.issuploadify, function(ind, ele) {
